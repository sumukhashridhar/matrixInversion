...
def create_output_directories() -> tuple:
    ...
    # if cublas_results is found then delete it
    if os.path.exists("cublas_results"):
        subprocess.run(["rm", "-rf", "cublas_results"])
        # sys.exit("cublas_results directory found. Deleted it. Please run the script again.")

    # Create directories for storing results
    base_dir = os.path.join(os.getcwd(), "cublas_results")
...
#     subprocess.run(" module load cuda-11.6.0 openmpi-4.0.2 binutils-2.41-static", shell=True, check=True, executable='/bin/bash')
...
def main():
    ...
    base_dir, cublas_dir, runtime_dir = create_output_directories()
    ...
    try:
        for num_matrices in num_matrices_list:
            file_name = f"cublas_results_{matrix_names[idx]}.json"
            idx += 1
            for matrix_size in matrix_sizes:
                ...
                runtime_file = os.path.join(runtime_dir, file_name)
