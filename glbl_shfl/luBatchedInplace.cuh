...
template<typename T>
__device__ void comp_U_shfl(T* A, int rowIdx, int matrixSize, int threadsPerMatrix, int mtrxOffset) {
        ...
        for (int colIdx = rowIdx; colIdx < matrixSize; colIdx++) {
            ...
            for (int l = threadIdx.x % threadsPerMatrix; l < rowIdx; l += threadsPerMatrix) {
                ...
                sum += A[rowIdx * matrixSize + l + mtrxOffset] * A[l * matrixSize + colIdx + mtrxOffset];
            }
            ...
            if (threadIdx.x % threadsPerMatrix == 0) {
                A[rowIdx * matrixSize + colIdx + mtrxOffset] -= sum;
...
template<typename T>
__device__ void comp_L_shfl(T* A, int rowIdx, int matrixSize, int threadsPerMatrix, int mtrxOffset) {
        ...
        for (int colIdx = rowIdx+1; colIdx < matrixSize; colIdx++) {
            ...
            for (int l = threadIdx.x % threadsPerMatrix; l < rowIdx; l += threadsPerMatrix) {
                ...
                sum += A[colIdx * matrixSize + l + mtrxOffset] * A[l * matrixSize + rowIdx + mtrxOffset];
            }
            ...
            if (threadIdx.x % threadsPerMatrix == 0) {
                A[colIdx * matrixSize + rowIdx + mtrxOffset] = (A[colIdx * matrixSize + rowIdx + mtrxOffset] - sum) / A[rowIdx * matrixSize + rowIdx + mtrxOffset];
...
template<typename T>
__device__ void inversion(T* A, int rowIdx, int matrixSize, int mtrxOffset) {
    // if (rowIdx < matrixSize) {
        ...
        for (int i = 0; i < matrixSize; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                sum += A[(i * matrixSize) + j + mtrxOffset] * y[j];
            }
        ...
        for (int i = matrixSize - 1; i >= 0; i--) {
            ...
            for (int j = i + 1; j < matrixSize; j++) {
                ...
                sum += A[(i * matrixSize) + j + mtrxOffset] * A[(j * matrixSize) + rowIdx + mtrxOffset];
            }
            // A[calc_swl_idx_lei(i * matrixSize + rowIdx, matrixSize, matrixIdInBlock, matrixSize * matrixSize)] = (y[i] - sum) / A[(i * matrixSize) + i];
            A[(i * matrixSize) + rowIdx + mtrxOffset] = (y[i] - sum) / A[(i * matrixSize) + i + mtrxOffset]; 
        }
    // }
...
__global__ void batched_lu_subwarp(T* A, int matrixSize, int numMatrices, int threadsPerMatrix, int matricesPerBlock) {
    ...
    if (globalMatrixId < numMatrices) {
        ...
        for (int rowIdx = 0; rowIdx < matrixSize; rowIdx++) {
            ...
            comp_U_shfl(A, rowIdx, matrixSize, threadsPerMatrix, mtrxOffset);
            ...
            comp_L_shfl(A, rowIdx, matrixSize, threadsPerMatrix, mtrxOffset);
        ...
        for (int rowIdx = threadIdInMatrix; rowIdx < matrixSize; rowIdx += threadsPerMatrix) {
            inversion(A, rowIdx, matrixSize, mtrxOffset);
        }
        __syncthreads();
